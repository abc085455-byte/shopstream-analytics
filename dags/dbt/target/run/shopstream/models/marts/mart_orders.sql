...
create or replace transient table SHOPSTREAM_DB.STAGING_marts.mart_orders
...
WITH orders AS (
    SELECT * FROM SHOPSTREAM_DB.STAGING_intermediate.int_orders_enriched
),

customers AS (
    SELECT * FROM SHOPSTREAM_DB.STAGING_staging.stg_customers
),
...
order_items_agg AS (
    ...
    FROM SHOPSTREAM_DB.RAW.order_items
